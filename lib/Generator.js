var File = require('raptor/files/File'),
    ...
    logger = require('raptor/logging').logger('raptor-static-site-generator'),
...
raptor.extend(Generator.prototype, {
    ...
    includeFilenamesInUrls: function(includeFilenamesInUrls) {
        this._site._includeFilenamesInUrls = includeFilenamesInUrls;
        return this;
    },
            ...
            templating.renderAsync(template, templateData, context)
                .then(
                    function() {
                        ...
                        _this.emit('afterRenderPage', page);

                        if (_this._pageWriterFunc) {
                            var promise = _this._pageWriterFunc(page);
                            if (promise) {
                                promise.then(
                                    function() {
                                        _this._pageFinished();
